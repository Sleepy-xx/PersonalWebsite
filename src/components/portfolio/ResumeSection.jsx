...
const education = [
  {
    ...
    college: '/PersonalWebsite/HKUST_symbol.svg',
    location: 'Hong Kong',
    period: '2023 - Present',
...
export default function ResumeSection() {
  ...
  // 1. 定义来自 globals.css 的样式变量
  
  // 对应 var(--border-gradient-onyx)
  // 这是一个从左上角(灰色)到右下角(透明)的渐变，形成了微妙的边框光感
  const borderGradient = 'linear-gradient(to bottom right, hsl(0, 0%, 25%) 0%, hsla(0, 0%, 25%, 0) 50%)';

  // 对应 var(--bg-gradient-jet)
  // 这是一个深色底色 + 左上角高光层的复合背景
        ...
        <ol className="ml-14">
          {education.map((item, index) => (
            <li key={index} className="relative flex flex-col mb-5 last:mb-0">
              ...
              {/* --- 核心卡片结构 --- */}
              {/* 外层：负责显示渐变边框 (Padding 1px 模拟边框) */}
              <div 
                className="relative rounded-2xl p-[1px] z-10"
                style={{ 
              ...
              >
                {/* 内层：负责显示内容和深色背景 */}
                <div 
                  className="rounded-2xl p-5 md:p-6 h-full w-full"
